...
def get_serial_dev_name() -> str:
   ...
   elif sys.platform == "win32": # Windows Subsystem for Linux
    #    print(platform.uname().release.lower())
       # List the serial devices available
       try:
           stdout = subprocess.check_output("powershell -Command \"[System.IO.Ports.SerialPort]::getportnames()\"",
       ...
       serial_dev_name = re.search("COM([0-9]*)", stdout)
       if serial_dev_name:
           serial_dev_name = f"COM{serial_dev_name.group(1)}"
